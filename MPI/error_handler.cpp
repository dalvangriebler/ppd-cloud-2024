 ...
 *  Description: This is a simple example of Master/Slave pattern in MPI programs
 *  File Name: error_handler.cpp
 *  Version: 1.0 (02/06/2018)
 *  Compilation Command: mpic++ -std=c++1y -Wall error_handler.cpp -o exe
...
void error_handler(int mpi_status){
    if (mpi_status != MPI_SUCCESS){
        int len;
        char err_string [MPI_MAX_ERROR_STRING];
        MPI_Error_string(mpi_status, err_string, &len);
        std::cerr << " MPI Error : (" << mpi_status << ") " << err_string << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
int main(int argc, char *argv []){
    ...
    if (myrank == MASTER_RANK) {
       for(int source = 1; source < numprocs; source++){
            error_handler(MPI_Send(&source, 1, MPI_INT, source, MESSAGE_TAG, MPI_COMM_WORLD));
            std::cout << "I am the Master, sending to " << source << " this integer: " << source << std::endl;
        }
    }else{
        MPI_Status status;
        int message=0;
        error_handler(MPI_Recv(&message, 1, MPI_INT, MASTER_RANK, MESSAGE_TAG, MPI_COMM_WORLD, &status));
